...
public class PageUtil {
	...
	public static String generate(int page, int size, int length, String url) {
		...
		if (startPage > MAX_PAGE_INDEX) {
			...
			//header.append(url);
			//header.append(pref);
			header.append("javascript:getHistory(1)");
			header.append("'>");
			header.append("First");
			header.append("</a>&nbsp;");
		...
		if (page > 1) {
			...
			//header.append(url);
			//header.append(pref);
			header.append("javascript:getHistory(" + (page - 1)+")");
			header.append("'>");
			header.append("Prev");
			header.append("</a>&nbsp;");
		}
		...
		if (startPage > MAX_PAGE_INDEX) {
			...
			header.append("javascript:getHistory(" + (prev - 1)+")");
			header.append("'>");
			header.append("[...]");
			header.append("</a>&nbsp;");
		}
		...
		for (int i = startPage; i <= endPage; i++) {
			if (i != page) {
				...
				//header.append(url);
				//header.append(pref);
				header.append("javascript:getHistory(" + (i)+")");
		...
		if (currentPageCount < lastPageCount) {
			...
			header.append("javascript:getHistory(" + (next)+")");
			header.append("'>");
			header.append("[...]");
			header.append("</a>&nbsp;");
		}
		...
		if (currentPageCount < lastPageCount) {
			...
			header.append("javascript:getHistory(" + totalPage+")");
			header.append("'>");
			header.append("Last");
			header.append("</a>&nbsp;");
		}
	...
	public static void main(String args[]) {
		String pageTag=PageUtil.generate(1, 10, 3,"/");
